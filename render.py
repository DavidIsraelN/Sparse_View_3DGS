...
def render_set(source_path, model_path, name, iteration, views, scene, gaussians, pipeline, background, use_mask,
    ...
    for cam in scene.train_cameras[1.0]:
        gt_img, _ = cam.get_image(with_mask = False)
        gt_img = ((gt_img).permute(1,2,0).clamp(0,1)[:,:,[2,1,0]]*255).detach().cpu().numpy().astype(np.uint8)
        gt_name = cam.image_name
        cv2.imwrite(os.path.join(gts_path, gt_name + ".jpg"), gt_img)
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        depth = out["plane_depth"].squeeze() * mask_gt.detach().squeeze() 
        depth_tsdf = depth.clone()
        depth = depth.detach().cpu().numpy().clip(0, max_depth)
        depth_i = (depth - depth.min()) / (depth.max() - depth.min() + 1e-20)
        depth_i = (depth_i * 255).clip(0, 255).astype(np.uint8)
        depth_color = cv2.applyColorMap(depth_i, cv2.COLORMAP_JET)

        mask_np = mask_gt.permute(1, 2, 0).detach().cpu().numpy()  # (H, W, 1)
        mask_3ch = np.repeat(mask_np, 3, axis=2)  # (H, W, 3)
        depth_color = (depth_color.astype(np.float32) * mask_3ch +
                    127.5 * (1 - mask_3ch)).astype(np.uint8)

        normal = out["rendered_normal"].permute(1,2,0)
        normal = normal/(normal.norm(dim=-1, keepdim=True)+1.0e-8)
        normal = normal.detach().cpu().numpy()
        normal = ((normal * mask_gt.permute(1,2,0).detach().cpu().numpy() + 1) * 127.5).astype(np.uint8).clip(0, 255)
        ...
        if use_depth_filter:
            ...
            depth_tsdf[mask] = 0
            print("wxs")
        depths_tsdf_fusion[view.image_name] = depth_tsdf.squeeze()

    if volume is not None:
        for _, view in enumerate(tqdm(views, desc="TSDF Fusion progress")):
            ref_depth = depths_tsdf_fusion[view.image_name]
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool,
    ...
    with torch.no_grad():
        ...
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False, prepare_data=False, task="render")
        ...
        if not skip_train:
            render_set(dataset.source_path, dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), scene, gaussians, pipeline, background, use_mask, 
        ...
        if not skip_test:
            render_set(dataset.source_path, dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), scene, gaussians, pipeline, background, use_mask)
...
if __name__ == "__main__":
    ...
    print(f"multi_view_num_ncc {model.multi_view_num_ncc}")
